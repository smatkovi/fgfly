...
// ---- merged shader object: /home/defaultuser/.local/share/harbour-fgview/fgdata/Compositor/Shaders/Default/urban.frag
// -*- mode: C; -*-
// Licence: GPL v2
...
void QDM(inout vec3 p, inout vec3 v)
{
    ...
    const float NODE_COUNT = float(TEXTURE_PIX_COUNT);
    ...
    while (level >= 0.0 && GlobalIterationCount < gIterationCap)
    {
        ...
        d = textureLod(QDMTex, uv.xy, uv.w).w;
    ...
    dA = textureLod(NormalTex, p2a.xy, p2a.w).w;
    dB = textureLod(NormalTex, p2b.xy, p2b.w).w;
...
float ray_intersect_relief(vec2 dp, vec2 ds)
{
    ...
    {
        depth += size;
        float t = step(0.95, texture(NormalTex, dp + ds * depth).a);
        if(best_depth > 0.996)
            if(depth >= t)
                best_depth = depth;
    }
...
void main (void)
{
    ...
    vec3 l = osg_LightSource0_position.xyz;
    vec3 diffuse = (gl_FrontFacing ? osg_FrontColor : osg_BackColor).rgb * max(0.0, dot(N, l));
    ...
    if ( shadow_factor < 1.0 )
        ambient_light = constantColor + osg_LightSource0_diffuse * shadow_factor * vec4(diffuse, 1.0);
    float emission_factor = (1.0 - smoothstep(0.15, 0.25, reflectance)) * emis;
    vec4 tc = texture(BaseTex, uv);
    emission_factor *= 0.5*pow(tc.r+0.8*tc.g+0.2*tc.b, 2.0) -0.2;
    ambient_light += (emission_factor * vec4(night_color, 0.0));

    vec4 noisevec   = texture(NoiseTex, (rawpos.xyz)*0.01*scale);
    vec4 nvL   = texture(NoiseTex, (rawpos.xyz)*0.00066*scale);
    ...
    vec4 finalColor = texture(BaseTex, uv);
    ...
    osg_FragColor = finalColor;

    if (dot(normal,-V) > 0.1) {
        vec4 iproj = osg_ProjectionMatrix * p;
        iproj /= iproj.w;
