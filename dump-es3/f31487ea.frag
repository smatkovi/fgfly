...
void main (void)
{
    ...
    vec4 nmap  = texture(NormalTex, osg_TexCoord[0].st * 8.0);
    vec4 map   = texture(Map, osg_TexCoord[0].st * 8.0);
    vec4 specNoise = texture(Noise, rawpos.xyz * 0.0045);
    vec4 noisevec = texture(Noise, rawpos.xyz);
    ...
    vec4 rainbow = texture(Rainbow, vec2(v, 0.0));

    // Map a fresnel effect
    vec4 fresnel = texture(Fresnel, vec2(v, 0.0));

    // map the refection of the environment
    vec4 reflection = texture(Environment, reflVec * dot(N,VNormal));
