...
void main (void)
{
	...
	vec4 nmap       = texture2D(NormalTex, osg_TexCoord[0].st * nmap_tile);
	vec4 reflmap    = texture2D(ReflMapTex, osg_TexCoord[0].st);
	vec4 noisevec   = texture3D(ReflNoiseTex, rawpos.xyz);
	vec4 lightmapTexel = texture2D(LightMapTex, osg_TexCoord[0].st);
