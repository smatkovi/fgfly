...
void QDM(inout vec3 p, inout vec3 v)
{
    ...
    while (level >= 0.0 && GlobalIterationCount < gIterationCap)
    {
        ...
        d = texture2DLodEXT(QDMTex, uv.xy, uv.w).w;
    ...
    dA = texture2DLodEXT(NormalTex, p2a.xy, p2a.w).w;
    dB = texture2DLodEXT(NormalTex, p2b.xy, p2b.w).w;
...
void main (void)
{
    ...
    if (dot(normal,-V) > 0.1) {
        ...
        iproj /= iproj.w;
        gl_FragDepthEXT = (iproj.z+1.0)/2.0;
    } else {
